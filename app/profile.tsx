...
import { User, Settings, Bell, CreditCard, CircleHelp as HelpCircle, Shield, LogOut, ChevronRight, QrCode, ScanLine, Moon, Sun, CreditCard as Edit, Star, Award, MapPin, Phone, Mail, Calendar, Ticket, MessageCircle, ChartBar as BarChart3, ArrowLeft } from 'lucide-react-native';
...
export default function ProfileScreen() {
  ...
  const { getUpcomingTickets, getPastTickets } = useBooking();
  ...
  // User stats (mock data) - Different for personal vs business
  const userStats = {
    eventsAttended: userType === 'personal' ? getPastTickets().length : 0,
    eventsCreated: userType === 'business' ? 8 : 0,
    totalBookings: userType === 'personal' ? (getUpcomingTickets().length + getPastTickets().length) : (userType === 'business' ? 156 : 0),
    rating: userType === 'business' ? 4.8 : 0,
  };
  ...
  const menuItems = [
    {
      ...
      show: userType === 'business', // Only show for business users
    },
    {
      id: 'settings',
  ...
  const quickActions = [
    {
      id: 'tickets',
      title: language === 'ar' ? 'تذاكري' : 'My Tickets',
      icon: QrCode,
      color: colors.primary,
      count: userType === 'personal' ? getUpcomingTickets().length : 0,
      action: () => router.push('/bookings'),
      show: userType === 'personal', // Only for personal users
    },
    {
      id: 'events',
      ...
      icon: Calendar,
      color: '#10b981',
      count: userType === 'business' ? userStats.eventsCreated : userStats.eventsAttended,
      action: () => router.push(userType === 'business' ? '/dashboard' : '/events'),
      show: true,
    },
    {
      id: 'messages',
    ...
    },
    {
      ...
      show: userType === 'business', // Only for business users
    },
  ];
        ...
        <View style={[dynamicStyles.profileCard, isRTL && styles.profileCardRTL]}>
          ...
          <View style={[styles.profileInfo, isRTL && styles.profileInfoRTL]}>
            <Text style={[dynamicStyles.profileName, isRTL && styles.textRTL]}>
              {user?.businessName || user?.name || user?.username || 'User Name'}
            </Text>
            ...
            {user?.bio && (
              <Text style={[dynamicStyles.profileBio, isRTL && styles.textRTL]}>
                {user.bio}
              </Text>
            )}
          </View>
            ...
            <>
              <View style={dynamicStyles.statsCard}>
                ...
                <Text style={[dynamicStyles.statLabel, isRTL && styles.textRTL]}>
                  {language === 'ar' ? 'فعاليات حضرتها' : 'Events Attended'}
                </Text>
              </View>
              <View style={dynamicStyles.statsCard}>
                <Text style={[dynamicStyles.statNumber, isRTL && styles.textRTL]}>
