...
export default function BusinessCreateEventScreen() {
  ...
  const handleAddSpeaker = () => {
    if (speakers.length >= 10) {
      Alert.alert('Error', 'Maximum 10 speakers allowed');
      return;
    }
  ...
  const handleAddSpeakerImage = (speakerId: string) => {
    Alert.alert(
      language === 'ar' ? 'إضافة صورة المتحدث' : 'Add Speaker Image',
      language === 'ar' ? 'اختر مصدر الصورة' : 'Choose image source',
      [
        {
          text: language === 'ar' ? 'الكاميرا' : 'Camera',
          onPress: () => addMockSpeakerImage(speakerId),
        },
        {
          text: language === 'ar' ? 'المعرض' : 'Gallery',
          onPress: () => addMockSpeakerImage(speakerId),
  ...
  const addMockSpeakerImage = (speakerId: string) => {
    // Mock speaker images
    const mockSpeakerImages = [
    ...
    const randomImage = mockSpeakerImages[Math.floor(Math.random() * mockSpeakerImages.length)];
    
    handleSpeakerChange(speakerId, 'image', randomImage);
  };
  ...
  const validateForm = () => {
    ...
    for (const speaker of speakers) {
      if (speaker.name && (!speaker.title || !speaker.bio)) {
        Alert.alert('Error', 'Please complete all speaker information or remove incomplete speakers');
        return false;
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    setTimeout(() => {
      ...
      Alert.alert('Success', 'Event created successfully!', [
        { text: 'OK', onPress: () => router.push('/(business-tabs)/events') }
      ]);
    }, 2000);
  };
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <Animated.View entering={FadeInDown.delay(650)} style={styles.section}>
          <View style={[styles.sectionHeader, isRTL && styles.sectionHeaderRTL]}>
            ...
            <TouchableOpacity style={styles.addSpeakerButton} onPress={handleAddSpeaker}>
              <Plus size={16} color="#a855f7" />
              <Text style={[styles.addSpeakerText, isRTL && styles.textRTL]}>
          ...
          {speakers.length === 0 ? (
            <View style={styles.noSpeakersContainer}>
              <Mic size={32} color="#d1d5db" />
              <Text style={[styles.noSpeakersText, isRTL && styles.textRTL]}>
                {language === 'ar' ? 'لم يتم إضافة متحدثين بعد' : 'No speakers added yet'}
              </Text>
              <Text style={[styles.noSpeakersSubtext, isRTL && styles.textRTL]}>
                {language === 'ar' ? 'أضف متحدثين لجعل فعاليتك أكثر جاذبية' : 'Add speakers to make your event more attractive'}
              </Text>
            </View>
          ) : (
            speakers.map((speaker, index) => (
              <View key={speaker.id} style={styles.speakerCard}>
                <View style={[styles.speakerHeader, isRTL && styles.speakerHeaderRTL]}>
                  <View style={[styles.speakerNumber, isRTL && styles.speakerNumberRTL]}>
                    <User size={16} color="#a855f7" />
                    <Text style={[styles.speakerNumberText, isRTL && styles.textRTL]}>
                      {language === 'ar' ? `المتحدث ${index + 1}` : `Speaker ${index + 1}`}
                    </Text>
                  </View>
                  <TouchableOpacity
                ...
                {/* Speaker Image */}
                <View style={styles.speakerImageSection}>
                  {speaker.image ? (
                    <View style={styles.speakerImageContainer}>
                      ...
                      <TouchableOpacity
                        ...
                        onPress={() => handleAddSpeakerImage(speaker.id)}
                      >
                    ...
                    <TouchableOpacity
                      style={styles.addSpeakerImageButton}
                      onPress={() => handleAddSpeakerImage(speaker.id)}
                    >
                      <Camera size={20} color="#a855f7" />
                      <Text style={[styles.addSpeakerImageText, isRTL && styles.textRTL]}>
                ...
                {/* Speaker Name */}
                <TextInput
                  style={[styles.speakerInput, isRTL && styles.inputRTL]}
                  placeholder={language === 'ar' ? 'اسم المتحدث' : 'Speaker Name'}
                  value={speaker.name}
                  onChangeText={(value) => handleSpeakerChange(speaker.id, 'name', value)}
                  placeholderTextColor="#999"
                />
                ...
                <TextInput
                  style={[styles.speakerInput, isRTL && styles.inputRTL]}
                  placeholder={language === 'ar' ? 'المنصب أو التخصص' : 'Title or Expertise'}
                  value={speaker.title}
                  onChangeText={(value) => handleSpeakerChange(speaker.id, 'title', value)}
                  placeholderTextColor="#999"
                />

                {/* Speaker Bio */}
                <TextInput
                  style={[styles.speakerBioInput, isRTL && styles.inputRTL]}
                  placeholder={language === 'ar' ? 'نبذة مختصرة عن المتحدث' : 'Brief bio about the speaker'}
                  value={speaker.bio}
                  onChangeText={(value) => handleSpeakerChange(speaker.id, 'bio', value)}
                  placeholderTextColor="#999"
                  multiline
                  numberOfLines={3}
                  maxLength={500}
                />
                <Text style={[styles.bioCharCount, isRTL && styles.textRTL]}>
        ...
        {/* Submit Button */}
        <Animated.View entering={FadeInDown.delay(850)} style={styles.submitSection}>
          <TouchableOpacity
